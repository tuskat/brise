...
export function downloadFormatFile(content, format, fileIndex, filesPerCall) {
  ...
  const filename =
    filesPerCall > 1 ? "output-" + fileIndex + "." + ext : "output." + ext;
  downloadBlob(content, filename);
}


// ═══════════════════════════════════════════════════════════
// SVG RENDERER — sanitizes and renders SVG content safely
// ═══════════════════════════════════════════════════════════
export function renderSVGBlock(containerEl, svgContent) {
  if (!containerEl || !svgContent) return;
  
  // Sanitize SVG with DOMPurify
  const sanitized = DOMPurify.sanitize(svgContent, {
    USE_PROFILES: { svg: true },
    ADD_TAGS: ['style'],
    ADD_ATTR: ['style', 'viewBox', 'xmlns', 'fill', 'stroke', 'stroke-width', 'transform', 'opacity', 'gradientUnits', 'x1', 'y1', 'x2', 'y2', 'r', 'cx', 'cy', 'rx', 'ry', 'width', 'height', 'x', 'y', 'points', 'd', 'text-anchor', 'dominant-baseline'],
    ALLOWED_URI_REGEXP: /^#?[w-]+$/ // Only allow local fragment identifiers
  });
...
export function renderAIResponse(containerEl, rawText) {
  ...
  while ((match = fenceRe.exec(processed)) !== null) {
    if (match.index > lastIdx) {
      parts.push({
        type: "text",
        content: processed.substring(lastIdx, match.index),
      });
    }
    ...
    if (part.type === "code") {
      ...
      if (!isSvg) {
        ...
        const isThoughtBlock =
          (part.lang || "").toLowerCase() === "thought" ||
          (part.lang || "").toLowerCase() === "thinking";
        langSpan.textContent = isThoughtBlock
          ? t("codeBlock.aiThoughts")
          : part.lang || t("codeBlock.code");
        ...
        // Render SVG content safely using renderSVGBlock
        const header = document.createElement("div");
        header.className = "detail-code-block-header";

        const langSpan = document.createElement("span");
        langSpan.className = "detail-code-block-lang";
        langSpan.textContent = "svg";
        header.appendChild(langSpan);
        ...
        svgContainer.className = "detail-code-block-pre";
        block.appendChild(svgContainer);
        renderSVGBlock(svgContainer, part.content);
      }
...
export function renderMultiFileResponse(result, filesPerCall, callNum, opts) {
  ...
  responseTitle &&
    (responseTitle.textContent =
      t("pg.response") + " (" + result.files.length + " files)");
  ...
  result.files.forEach((file) => {
    ...
    const filename =
      filesPerCall > 1 ? "output-" + file.index + "." + ext : "output." + ext;

    if (file.download_ready) {
      card.innerHTML =
        '<div class="pg-file-card-header">' +
        '<span class="pg-file-card-name">' +
        escapeHtml(filename) +
        "</span>" +
        '<button class="pg-file-card-download-btn" data-index="' +
        file.index +
        '">' +
        t("general.download") +
        "</button>" +
        "</div>" +
        '<pre class="pg-file-card-content">' +
        escapeHtml(file.data) +
        "</pre>";

      card
        .querySelector(".pg-file-card-download-btn")
      ...
      card.innerHTML =
        '<div class="pg-file-card-header">' +
        '<span class="pg-file-card-name pg-file-card-name-error">' +
        escapeHtml(filename) +
        "</span>" +
        "</div>" +
        '<pre class="pg-file-card-content pg-file-card-content-error">' +
        escapeHtml(file.error || file.raw || t("general.error")) +
        "</pre>";
    }
...
export function createHistoryItem(item, onDetail) {
  ...
  if (item.format) {
    formatBadge =
      '<span class="badge badge-caps badge-accent self-end">' +
  ...
  div.innerHTML =
    ...
    '<span class="w-100 t-caption t-weight-500 c-secondary">' +
    formatTime(item.timestamp) +
    "</span>" +
    formatBadge +
    '<span class="t-body t-weight-700 self-end ' +
    statusClass +
    '">' +
    statusIcon +
    "</span>" +
    ...
    '<span class="c-accent">' +
    (item.persona_id || t("general.default")) +
    "</span>" +
    '<span class="t-caption-mono">' +
    item.latency +
    "ms</span>" +
    "</div>" +
    '<p class="mt-md truncate c-secondary t-body">' +
    truncate(item.user_prompt, 80) +
    "</p>";
...
export function renderHistoryList(historyList, history, onDetail) {
  ...
  if (history.length === 0) {
    historyList.innerHTML =
      '<p class="placeholder">' + t("pg.noHistory") + "</p>";
    return;
  }
...
export function renderDetailView(item, detailContent, detailView, onDownload) {
  ...
  if (item.format) {
    formatSection =
      ...
      '<h3 class="label label-sm">' +
      t("history.format") +
      "</h3>" +
      '<p class="t-body c-primary"><span class="badge badge-caps badge-accent">' +
      escapeHtml(item.format) +
      "</span></p>" +
      "</div>";
    ...
    for (let f = 1; f <= filesCount; f++) {
      ...
      const fname =
        filesCount > 1 ? "output-" + f + "." + ext : "output." + ext;
      downloadBtns +=
        '<button class="btn btn-primary btn-sm" data-history-id="' +
        item.id +
        '" data-file="' +
        f +
        '">' +
  ...
  detailContent.innerHTML =
    ...
    "</h3>" +
    '<p class="t-body c-primary m-0 ' +
    (item.status === "success" ? "c-success" : "c-error") +
    '">' +
    ...
    '<div class="flex-col gap-xs">' +
    '<h3 class="label label-sm">' +
    t("history.userPrompt") +
    "</h3>" +
    '<div class="text-block text-block-prompt">' +
    escapeHtml(item.user_prompt) +
    "</div>" +
    "</div>" +
    '<div class="flex-col gap-xs">' +
    '<h3 class="label label-sm">' +
    t("history.aiResponse") +
    "</h3>" +
